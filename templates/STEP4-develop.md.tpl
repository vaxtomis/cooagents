...
7. 若实现必须偏离 Step2/Step3 的计划或执行地图，继续使用更正确的实现路径，但必须在 findings JSON 的 `deviations` 中记录 WHAT/WHY。
...
9. 将自审结果写入 `$findings_output_path`，必须是合法 JSON：
   ...
   ```
   `pass=true` 表示已开展的 lint / typecheck / 测试均通过，且没有已知阻断；已运行检查失败即 `pass=false` 并在 `findings` 里列出至少一条。未运行建议测试时，必须在 `findings` 说明原因。`plan_execution[].status` 仅可为 `done`、`deferred`、`blocked`。`package_manager`、`gitignore_maintenance`、`validation_commands`、`deviations` 是可选扩展字段，但建议尽量填写。
