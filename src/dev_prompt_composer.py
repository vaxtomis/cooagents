...
_STEP_WALL_STEP3 = (
    ...
    "「在哪里改、哪些模式可镜像/需适配/应避免、如何验证」。\n"
    "**唯一输出**：在 ctx-round-N.md 写入三段 H2"
    "（浓缩上下文 / 模式镜像 / 执行地图）。\n"
    "**推荐做法**：可用一两句话简述相关代码逻辑；先判断现有模式是否健康，"
    "不要把旧代码、过时实现或 MVP 临时写法默认当成最佳实践；**强烈推荐**"
...
_CONTEXT_COMPLETENESS_GUIDE = (
    ...
    "说明计划上下文不足，应降低 `plan_score_a`。同时检查 Step3 `## 模式镜像` "
    "是否先做模式健康度判断：健康模式标为 `MIRROR`，旧模式或 MVP 简化写法标为 "
    "`ADAPT` 并给出边界，反模式/过时/安全或数据风险写法标为 `AVOID` 并给出替代路径。"
    "若 Step3 把明显错误模式当成必须镜像，降低 `plan_score_a`；若 Step4 因此复制了"
    "坏模式或无理由偏离 `ADAPT`/`AVOID` 边界，降低 `actual_score_b`，必要时归为 "
    "`impl_gap`。若上下文不足导致开发计划缺关键实现路径、接口约束或测试策略，"
    "优先归为 `req_gap`；只缺少少量证据引用时，可在 `issues` 或 `next_round_hints` "
    "中记录并轻微扣分。"
)
