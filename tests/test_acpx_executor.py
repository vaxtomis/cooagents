...
async def test_restore_on_startup_ticks_run_after_reconciling_dead_session(executor, db):
    """restore_on_startup should mark stale jobs interrupted without probing.

    After a process restart there is no _watch task to monitor the job, so
    all stale starting/running jobs are treated as interrupted regardless of
    queue owner status.
    """
    from datetime import datetime, timezone
    now = datetime.now(timezone.utc).isoformat()
