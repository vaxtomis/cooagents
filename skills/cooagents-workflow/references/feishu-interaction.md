...
1. `exec curl GET -H "X-Agent-Token: $AGENT_API_TOKEN" /api/v1/runs/{run_id}/artifacts` 获取产物列表。
2. 根据 gate 选择需要发送的文档正文：
   - `REQ_REVIEW`：最新 `kind=req`
   - `DESIGN_REVIEW`：最新 `kind=design`
   - `DEV_REVIEW`：最新 `kind=test-report`
3. `exec curl GET -H "X-Agent-Token: $AGENT_API_TOKEN" /api/v1/runs/{run_id}/artifacts/{artifact_id}/content` 获取完整正文。
